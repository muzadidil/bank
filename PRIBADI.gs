/**
 * ZASHA MUTASI - PROJECT REUSABLE (Updated Total Source)
 */

function doGet(e) {
  var bulan = e.parameter.bulan || "JAN_2023"; 
  var template = HtmlService.createTemplateFromFile('View_Pribadi');
  template.bulanAktif = bulan; 
  
  return template.evaluate()
      .setTitle('Laporan Pribadi - ' + bulan)
...
function getSheetData(namaBulan) {
  ...
  // 1. Ambil Data Transaksi dari Sheet Master (Misal: JAN_2023)
  ...
  // 2. Ambil Angka Total Khusus dari Sheet "PRIBADI" Sel J1
  const pribadiSheet = ss.getSheetByName("PRIBADI");
  const totalPribadiJ1 = pribadiSheet ? pribadiSheet.getRange("J1").getValue() : 0;
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (nominal && nominal !== "" && kategori === "PRIBADI") {
      filteredData.push({
        baris: i + 1,
  ...
  return {
    ...
    totalMaster: totalPribadiJ1 // Sekarang mengambil dari sheet PRIBADI!J1
  };
}
